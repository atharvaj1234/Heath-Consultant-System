...
initializeDatabase()
  .then(() => {
    ...
    app.post('/api/register', [
      ...
      body('role').isIn(['user', 'consultant', 'admin']).withMessage('Invalid role selected')
    ], async (req, res) => {
      ...
      try {
        ...
        db.run(sql, values, function (err) {
          ...
          // Create a consultant profile if the user registered as a consultant
          if (role === 'consultant') {
            db.run(
              "INSERT INTO consultants (userId, specialty, qualifications, availability, imageUrl) VALUES (?, ?, ?, ?, ?)",
              [userId, '', '', '', ''], // You can set default values here
              function (err) {
                if (err) {
                  return handleDatabaseError(res, err, 'Failed to create consultant profile');
                }
                console.log('Consultant profile created with userId:', userId);
                res.status(201).json({ id: userId, fullName, email, role, isConsultant: 1, isApproved: 0 });
              }
            );
          } else {
            res.status(201).json({ id: userId, fullName, email, role, isConsultant: 0, isApproved: 0 });
          }
        });
      } catch (error) {
        console.error(error);
    ...
    app.put('/api/consultant/profile', authenticateToken, [
      ...
      db.get("SELECT isConsultant FROM users WHERE id = ?", [userId], (err, user) => {
        ...
        db.run(
          "UPDATE consultants SET specialty = ?, qualifications = ?, availability = ? WHERE userId = ?",
    ...
    app.get('/api/admin/users', authenticateAdmin, (req, res) => {
      ...
      db.all("SELECT id, fullName, email, role, isConsultant, bloodGroup, medicalHistory, currentPrescriptions, contactInformation, areasOfExpertise, isApproved FROM users", [], (err, users) => {
        if (err) {
    ...
    app.get('/api/consultant/bookings', authenticateToken, (req, res) => {
      ...
      db.get("SELECT isConsultant FROM users WHERE id = ?", [userId], (err, user) => {
        ...
        db.all("SELECT * FROM bookings WHERE consultantId IN (SELECT id FROM consultants WHERE userId = ?)", [userId], (err, bookings) => {
          if (err) {
            return handleDatabaseError(res, err, 'Failed to retrieve consultant bookings');
          }
          res.json(bookings);
        });
      });
    ...
    app.put('/api/admin/consultants/:userId/approve', authenticateAdmin, (req, res) => {
      const { userId } = req.params;
      const db = getDb();

      db.run("UPDATE users SET isApproved = 1 WHERE id = ?", [userId], function (err) {
        if (err) {
          return handleDatabaseError(res, err, 'Failed to approve consultant');
        }
        if (this.changes === 0) {
          return res.status(404).json({ message: 'Consultant not found' });
        }
      ...
      app.get('/api/consultants/:consultantId/bookings', authenticateToken, (req, res) => {
        const { consultantId } = req.params;
        const userId = req.user.userId;
        const db = getDb();

        // Verify if the requesting user is the consultant or admin
        db.get("SELECT userId FROM consultants WHERE id = ?", [consultantId], (err, consultant) => {
            if (err) {
                return handleDatabaseError(res, err, 'Failed to check consultant');
            }
            ...
            db.get("SELECT role FROM users WHERE id = ?", [userId], (err, user) => {
                ...
                //Allow Admin to get All consultant Bookings
                if (consultant.userId !== userId && user.role !== 'admin') {
                    return res.status(403).json({ message: 'Unauthorized access' });
                }

                // Get all bookings for the consultant
                db.all("SELECT * FROM bookings WHERE consultantId = ?", [consultantId], (err, bookings) => {
                    if (err) {
                        return handleDatabaseError(res, err, 'Failed to retrieve bookings');
                    }
                    res.json(bookings);
